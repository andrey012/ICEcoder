...
// ==============
	...
	findInFiles: function() {
		this.findInFilesScreen(false);
	},

	findInFilesScreen: function(hide) {
		if (!hide) {
			top.get('mediaContainer').innerHTML = '<iframe id="findInFilesForm" src="lib/find-in-files-screen.php" class="whiteGlow" style="width: 970px; height: 610px"></iframe>';
		} else {
			var iframe = document.getElementById('findInFilesForm');
	...
	moveFile: function(oldName,newName) {
		...
		if (newName) {
			...
			if (confirm("Are you sure you want to move file " + oldName + " to " + newName + " ?")){
